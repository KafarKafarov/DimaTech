...
@pytest.mark.asyncio
async def test_login_profile(client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_user_forbidden_admin(client: AsyncClient) -> None:
	...
	response = await client.get(
		'/api/v1/admin/users', headers=auth_headers(token=token)
	)
...
@pytest.mark.asyncio
async def test_admin_crud_user(client: AsyncClient) -> None:
...
@pytest.mark.asyncio
async def test_webhook_creates_account(client: AsyncClient) -> None:
	...
	signature = build_webhook_signature(
		payload=payload, secret_key='test-payment-secret'
	)
	...
	payments_response = await client.get(
		'/api/v1/users/me/payments', headers=user_headers
	)
	accounts_response = await client.get(
		'/api/v1/users/me/accounts', headers=user_headers
	)
...
@pytest.mark.asyncio
async def test_webhook_idempotent(client: AsyncClient) -> None:
